...
public class DynamoDBServiceImpl implements DynamoDBService {
    ...
    public String updateUser(Map<String, String> pathParameters, String inputBody) {
        try {
            ...
            if (existingUser != null) {
                // Обновляем только те атрибуты пользователя, которые присутствуют в запросе
                updateUsersNotNullAttributes(existingUser, updatedUser);
                dynamoDBMapper.save(existingUser);
                return getJsonResponse("User updated successfully");
            } else {
                return getJsonResponse("User not found");
            }
        } catch (Exception e) {
            return getJsonResponse("Error updating user: " + e.getMessage());
        }
    }
